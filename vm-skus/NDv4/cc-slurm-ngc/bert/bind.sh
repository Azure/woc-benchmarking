...
case "${cluster}" in
   ...
   selene)
        ...
        ibdevs=(mlx5_0 mlx5_1 mlx5_2 mlx5_3 mlx5_4 mlx5_5 mlx5_6 mlx5_7)
	;;
    azure)
        # Need to specialize for azure
        ibdevs=(ibP257p0s0 ibP258p0s0 ibP259p0s0 ibP260p0s0 ibP261p0s0 ibP262p0s0 ibP263p0s0 ibP264p0s0)
        ;;
    '')
...
case "${cpu_mode}" in
	...
	numactl_args+=( "--cpunodebind=24-35:36-47:0-11:12-23:72-83:84-95:48-59:60-71" )
        ;;
    *.sh)
	source "${cpu_mode}"
...
case "${mem_mode}" in
	...
	numactl_args+=( "--membind=1:1:0:0:3:3:2:2" )
        ;;
    *.sh)
	source "${mem_mode}"
	if [ -z "${bind_mem:-}" ]; then
